...
from .models import Answer, Quizzes, Question, Attempter, Attempt
...
def NewQuiz(request, course_id, module_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            quiz = Quizzes.objects.create(user=user, title=title, description=description,
                                          due=due, allowed_attempts=allowed_attempts, time_limit_mins=time_limit_mins)
    ...
    return render(request, 'quiz/new-quiz.html', context)
...
def TakeQuiz(request, course_id, module_id, quiz_id):
    ...
    return render(request, 'quiz/take-quiz.html', context)
...
def SubmitAttempt(request, course_id, module_id, quiz_id):
    ...
    if request.method == 'POST':
        ...
        for q, a in zip(questions, answers):
            ...
            Completion.objects.create(user=user, course_id=course_id, quiz_id=quiz_id)
...
def AttemptDetail(request, course_id, module_id, quiz_id, attempt_id):
    ...
    return render(request, 'quiz/attempt-detail.html', context)
